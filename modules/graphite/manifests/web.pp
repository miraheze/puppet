...
class graphite::web(
    ...
    uwsgi::app { 'graphite-web':
        settings => {
            ...
            uwsgi => {
                ...
                'buffer-size' => 8192, # bump request buffer space
    ...
    exec { 'create_graphite_admin':
        command => "/usr/bin/python3 /usr/local/sbin/graphite-auth set ${admin_user} ${admin_pass}",
        unless  => "/usr/bin/python3 /usr/local/sbin/graphite-auth check ${admin_user} ${admin_pass}",
        user    => 'www-data',

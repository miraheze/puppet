...
if [ ! -d /srv/services/services/ ]; then
        cd /srv/services/ && GIT_SSH_COMMAND='ssh -o StrictHostKeyChecking=no -i /srv/services/id_ed25519 -F /dev/null' git clone git@github.com:miraheze/services.git && cd /srv/services/services/ && git config --local core.sshCommand "ssh -o StrictHostKeyChecking=no -i /srv/services/id_ed25519 -F /dev/null"
else

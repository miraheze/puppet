...
class mediawiki::dumps {
    ...
    $private_image_dump.each |$key, $value| {
        ...
        cron { "Export ${key} private images ${value}":
            ...
            command  => "/usr/bin/nice -n19 /bin/mkdir -p /mnt/mediawiki-static/private/dumps/${key} && /usr/bin/nice -n19 /bin/mkdir -p /mnt/mediawiki-static/private/dumps/${key}/images/ && /usr/bin/nice -n19 /usr/bin/zip -r /mnt/mediawiki-static/private/dumps/${key}/images/${key}.zip /mnt/mediawiki-static/${key}/ && /usr/bin/nice -n19 /usr/bin/php /srv/mediawiki/w/maintenance/importImages.php /mnt/mediawiki-static/private/dumps/${key}/images/ --comment='Import image zip dump for ${key}' --overwrite --wiki=${key} --extensions=zip",
            user     => 'www-data',
            minute   => '0',
            hour     => '0',
            month    => '*',

function role::trafficserver_caching_rules(Hash $req_handling, Hash $alternate_domains, Array[Trafficserver::Mapping_rule] $mapping_rules) >> Array[Trafficserver::Caching_rule] {
    $caching_rules = ($req_handling + $alternate_domains).map |$hostname, $entry| {
        if $entry['caching'] == 'pass' {
            ...
            if length($remap_rule) == 0 {
                ...
                fail("No rule found for ${hostname} in role::trafficserver::mapping_rules")
            }

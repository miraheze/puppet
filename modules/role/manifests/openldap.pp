...
class role::openldap (
    ...
    $firewall_rules = join(
        query_facts('Class[Role::Grafana] or Class[Role::Graylog] or Class[Role::Matomo] or (Class[Role::Mediawiki] and !facts['mediawiki::jobqueue::runner::beta']) or Class[Role::Openldap]', ['networking'])
        .map |$key, $value| {

...
class role::mediawiki_task (
    ...
    if $strict_firewall {
        ...
        $firewall_rules_str = join(
            $cloudflare_ipv4 + $cloudflare_ipv6 + query_facts('Class[Role::Mediawiki] or Class[Role::Mediawiki_task] or Class[Role::Varnish] or Class[Role::Icinga2] or Class[Role::Prometheus] or Class[Role::Bastion] or Class[Role::Eventgate]', ['networking'])
            .map |$key, $value| {

...
class role::cache::haproxy(
    ...
    Hash[String, Array] $varnish_backends = lookup('role::cache::haproxy::varnish_backends'),
    ...
    Hash[String, Array[Haproxy::Header]] $add_headers = lookup('role::cache::haproxy::add_headers'),
    ...
    Boolean $numa_networking = lookup('role::cache::haproxy::numa_networking', {'default_value'                                               => true}),
    ...
    $cache_backends.each | $name, $property | {
        monitoring::nrpe { "Haproxy Frontend for ${name}":

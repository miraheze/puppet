...
sub vcl_deliver {
	...
	set resp.http.Content-Security-Policy = "default-src 'self' 'unsafe-inline' 'unsafe-eval' blob: data: <%- @csp_whitelist.each_pair do |config, value| -%> <%= value %> <%- end -%>; frame-ancestors 'self' <%- @frame_whitelist.each_pair do |config, value| -%> <%= value %> <%- end -%>";
}

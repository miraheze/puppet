...
sub vcl_deliver {
	...
	set resp.http.Content-Security-Policy = "default-src 'self' data: *.miraheze.org *.wikimedia.org *.wikipedia.org *.wikibooks.org *.wiktionary.org *.wikiquote.org *.wikisource.org *.wikiversity.org *.wikinews.org mediawiki.org wikidata.org *.google.com *.gstatic.com *.addthis.com 'unsafe-inline' 'unsafe-eval'";
}

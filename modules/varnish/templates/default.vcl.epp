<%- | String $interval_check, String $interval_timeout, Hash $backends, Array[String] $cloudflare_ipv4, Array[String] $cloudflare_ipv6, String $debug_access_key, Hash $csp | -%>
...
<%- } -%>
<%- if $property['xdebug'] and (stdlib::start_with($name, 'mw') or stdlib::start_with($name, 'test')) { -%>
...
sub vcl_deliver {
	...
	if (resp.status != 304 && !resp.http.Content-Security-Policy) {
		set resp.http.Content-Security-Policy = "<%- $csp.each_pair |$type, $value| { %> <%= $type %> <%= $value.join(' ') %>; <%- } -%>";
	}

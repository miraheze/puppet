    ...
    def _handle_request_authentication(self, env, start_response):
        ...
        if match is None:
                ...
                if match:
                        ...
                        if match.group('path'):
                                obj = match.group('path') # <path>
                        elif match.group('proj') != 'public':
                                query = "limit=1&prefix={}%2F{}%2F".format(match.group('wiki'), match.group('proj'))
                        else:
                                query = "limit=1&prefix={}%2F".format(match.group('wiki'))
        ...
        if match is None:
                ...
                if match:
                        wiki = match.group('wiki') # <wiki>
                        if match.group('proj') and match.group('proj') != 'public':
                                proj = match.group('proj') # <proj>
                        obj = match.group('path') # <path>

        if match is None:
                match = re.match(
                        r'^/v1/AUTH_mw/miraheze-(?P<wiki>[^/]+)-mw\?.+$',
                        env['REQUEST_URI'])
                if match:
                        query_match = dict(urllib.parse.parse_qsl(env['QUERY_STRING']))
                        query = "limit={}".format(query_match['limit'])
        ...
        if match is None:
                match = re.match(
                        r'^/v1/AUTH_mw/miraheze-(?P<wiki>[^/]+)-mw-private\?.+$',
                        env['REQUEST_URI'])
                if match:
                        container = "miraheze-mw-private"
                        query_match = dict(urllib.parse.parse_qsl(env['QUERY_STRING']))
        ...
        if match:
            ...
            if query:
                ...
                if obj:
                    ...
                    newpath = "/v1/%s/%s/%s/%s" % (self.account, container, wiki, proj)
            else:
                if obj:

...
class _MirahezeRewriteContext(WSGIContext):
    ...
    def get_auth_token(self, env):
        if (_MirahezeRewriteContext.auth_token and
                _MirahezeRewriteContext.auth_token_expiry > monotonic.monotonic()):
            self.logger.debug('Reusing existing auth token for thumbnail expiry update')
            return _MirahezeRewriteContext.auth_token
        ...
        _MirahezeRewriteContext.auth_token = resp.headers['X-Auth-Token']

        expiry = int(resp.headers['X-Auth-Token-Expires'])
        _MirahezeRewriteContext.auth_token_expiry = monotonic.monotonic() + expiry
    ...
    def update_expiry(self, env):
        ...
        if resp.status_int == 401:
            ...
            _MirahezeRewriteContext.auth_token = False
            resp = self.post_expiry(env)

...
#   $auto_firewall      - If enabled and if $hosts_allow is set, generate a firewall service which restricts access to the allowed hosts
#   $auto_firewall_ipv6 - If auto_firewall is used and this option is enabled, firewall rules are also generated for ipv6
#
...
define rsync::server::module (
  ...
  if $auto_firewall and $hosts_allow {
      $hosts_allow_firewall = join($hosts_allow, ' ')

      firewall::service { "rsyncd_access_${name}":
          ensure => $ensure,
          proto  => 'tcp',
          port   => 873,
          srange => "@resolve((${hosts_allow_firewall}))",
      }
      ...
      if $auto_firewall_ipv6 {
          firewall::service { "rsyncd_access_${name}_ipv6":
              ensure => $ensure,
              proto  => 'tcp',
              port   => 873,
              srange => "@resolve((${hosts_allow_firewall}),AAAA)",
          }
          # rsync::server is always used with include semantics, so we must do this.
          if lookup('rsync::server::wrap_with_stunnel', {'default_value' => false}) {  # lint:ignore:wmf_styleguide

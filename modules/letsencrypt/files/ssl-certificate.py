    ...
    def generate_letsencrypt_certificate(self):
        if self.wildcard:
            ...
            if self.no_existing_key:
                os.system("/usr/bin/certbot --force-renewal --expand certonly --manual --preferred-challenges dns-01 {2} -d {0} {1}".format(self.domain, self.secondary_domain, self.overwrite))
            else:
                os.system("/usr/bin/certbot --force-renewal --reuse-key --expand certonly --manual --preferred-challenges dns-01 {2} -d {0} {1}".format(self.domain, self.secondary_domain, self.overwrite))
            ...
            if self.no_existing_key:
                os.system("/usr/bin/certbot {1} --noninteractive --force-renewal --expand certonly -a webroot {3} -d {0} {2}".format(self.domain, self.quiet, self.secondary_domain, self.overwrite))
            else:
                os.system("/usr/bin/certbot {1} --noninteractive --force-renewal --reuse-key --expand certonly -a webroot {3} -d {0} {2}".format(self.domain, self.quiet, self.secondary_domain, self.overwrite))
    ...
    def renew_letsencrypt_certificate(self):
        if self.wildcard:
            ...
            if self.no_existing_key:
                ...
                os.system("/usr/bin/certbot --expand certonly --manual --preferred-challenges dns-01 {2} -d {0} {1}".format(self.domain, self.secondary_domain, self.overwrite))

                os.system("/usr/bin/sed -i 's/reuse_key = False/reuse_key = True/g' /etc/letsencrypt/renewal/{0}.conf".format(self.domain))
            else:
                os.system("/usr/bin/certbot --reuse-key --expand certonly --manual --preferred-challenges dns-01 {2} -d {0} {1}".format(self.domain, self.secondary_domain, self.overwrite))
            ...
            if self.no_existing_key:
                ...
                os.system("/usr/bin/certbot --force-renewal --expand {1} --noninteractive renew --cert-name {0}".format(self.domain, self.quiet))

                os.system("/usr/bin/sed -i 's/reuse_key = False/reuse_key = True/g' /etc/letsencrypt/renewal/{0}.conf".format(self.domain))
            else:
                os.system("/usr/bin/certbot --force-renewal --reuse-key --expand {1} --noninteractive renew --cert-name {0}".format(self.domain, self.quiet))

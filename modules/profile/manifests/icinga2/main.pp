class profile::icinga2::main (
    String $icinga2_db_host                 = lookup('icinga_ido_db_host', 'db7.miraheze.org'),
    String $icinga2_db_name                 = lookup('icinga_ido_db_name', 'icinga'),
    String $icinga2_db_user                 = lookup('icinga_ido_user_name', 'icinga2'),
    String $ido_db_user_password            = lookup('passwords::icinga_ido'),
    String $mirahezebots_password           = lookup('passwords::irc::mirahezebots'),
    String $icingaweb2_db_host              = lookup('icingaweb_db_host', 'db7.miraheze.org'),
    String $icingaweb2_db_name              = lookup('icingaweb_db_name', 'icingaweb2'),
    String $icingaweb2_db_user_name         = lookup('icingaweb_user_name', 'icingaweb2'),
    String $icingaweb2_db_user_password     = lookup('passwords::icingaweb2'),
    String $icingaweb2_ido_db_host          = lookup('icinga_ido_db_host', 'db7.miraheze.org'),
    String $icingaweb2_ido_db_name          = lookup('icinga_ido_db_name', 'icinga'),
    String $icingaweb2_ido_db_user_name     = lookup('icinga_ido_user_name', 'icinga2'),
    String $icingaweb2_icinga_api_password  = lookup('passwords::icinga_api'),
    String $ticket_salt                     = lookup('passwords::ticket_salt', ''),
) {
    class { '::monitoring':
